...
def _build_runtime_graph(config: AutomationConfig, storage: PolicyHooks | Any, *, proxy: str | None = None) -> AutomationGraph:
    ...
    def draft_reply(snapshot: WorkflowStateModel):
        ...
        snapshot.log_event(
            "draft_reply",
            "deterministic fallback reply used",
            duration_ms=round((perf_counter() - started_at) * 1000, 2),
        )
...
def build_request_binding(
    ...
    *,
    ...
    if function_name == "run_repo_post":
        ...
        return request, None
    if function_name == "run_direct_post":
...
async def run_repo_post(*, run_id: str, job_id: str, payload: dict[str, Any], storage: Any) -> dict[str, Any]:
    request, proxy = build_request_binding("run_repo_post", run_id=run_id, job_id=job_id, payload=payload) or (
        None,
        None,
    )
    if request is None:
        raise ValueError("unsupported function binding: run_repo_post")
    return await _run_request(request, storage=storage, proxy=proxy)


async def run_direct_post(*, run_id: str, job_id: str, payload: dict[str, Any], storage: Any) -> dict[str, Any]:
    request, proxy = build_request_binding("run_direct_post", run_id=run_id, job_id=job_id, payload=payload) or (
        None,
        None,
    )
    if request is None:
        raise ValueError("unsupported function binding: run_direct_post")
    return await _run_request(request, storage=storage, proxy=proxy)
